...
async function hyperlink(
  ...
  function handleError(error) {
    ...
    if (operator) {
      ...
      const contentTypeMismatchReport = {
        ...
        at: [
          ...new Set(asset.incomingRelations.map(r => r.debugDescription))
        ].join('\n        ')
      };
    ...
    if (error.asset) {
      if (error.asset.incomingRelalations.length > 0) {
        report.at = error.asset.incomingRelations[0].debugDescription;
      }
    } else if (error.relation) {
      report.at = error.relation.debugDescription;
    }
  ...
  async function processAsset(asset) {
    if (!processedAssets.has(asset)) {
      ...
      if (asset.incomingRelations.length > 0) {
        loadReport.at = asset.incomingRelations[0].debugDescription;
      } else {
      ...
      for (const relation of asset.externalRelations) {
        if (asset.isLoaded) {
          relation.debugDescription = relationDebugDescription(relation);
        }
        ...
        if (fragment) {
          if (fragment === '#') {
            ...
            (relation.to.incomingFragments =
              ...
              fragment,
              relationDebugDescription: relation.debugDescription,
              href: relation.href,
      ...
      (function unloadButKeepRelations(asset) {
        asset._parseTree = asset._text = asset._rawSrc = undefined;
        if (asset._jsdom) {
          asset._jsdom.window.close();
          asset._jsdom = undefined;
        }
        for (const relation of asset.outgoingRelations) {
          if (relation.to.isInline && relation.to !== relation.from) {
            unloadButKeepRelations(relation.to);
            relation.node = relation.parentNode = relation.propertyNode = this.argumentNode = this.argumentsNode = undefined;
          }
        }
      })(asset);
    }
  ...
  function checkRedirectChainFrom(asset, isCycle) {
    ...
    let at;
    if (asset.incomingRelations.length > 0) {
      at = asset.incomingRelations[0].debugDescription;
    } else {
