...
module.exports = async function (options, tapInstance) {
    options = options || {};

    const t = tapInstance || new TapRender();
    ...
    function handleError(error) {
        ...
        if (error.asset) {
            if (error.asset._incomingRelationDescription) {
                report.at = error.asset._incomingRelationDescription;
            }
        ...
        async function processAsset(asset) {
            if (!processedAssets.has(asset)) {
                ...
                for (const relation of asset.externalRelations) {
                    ...
                    if (!relation.to._incomingRelationDescription) {
                        relation.to._incomingRelationDescription = relationDebugDescription(relation);
                    }
