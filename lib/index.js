...
async function hyperlink({
    ...
    function maybeUnloadParents(asset) {
        for (const incomingRelation of asset.incomingRelations) {
            const nonInlineAncestor = incomingRelation.from.nonInlineAncestor;
            if (nonInlineAncestor.externalRelations.every(e => processedAssets.has(e.to))) {
                nonInlineAncestor.unload();
            }
        }
        if (asset.externalRelations.length === 0) {
            asset.unload();
        }
    }
